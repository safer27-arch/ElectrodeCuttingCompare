...
public final class VibrationCompensatedAnalyzer {
    ...
    private static Bitmap draw(Result r,String pf){int w=1200,h=900,l=75,rr=1140,t=210,bot=600;Bitmap o=Bitmap.createBitmap(w,h,Bitmap.Config.ARGB_8888);Canvas c=new Canvas(o);c.drawColor(Color.WHITE);Paint p=new Paint(1);p.setColor(Color.rgb(15,48,88));p.setTextSize(38);p.setFakeBoldText(true);c.drawText("v0.8 Easy Diagnostic · Cutter vs Vibration",45,58,p);p.setFakeBoldText(false);p.setTextSize(23);p.setColor(Color.DKGRAY);c.drawText(pf+" · 고정부 흔들림 제거 후 Cutter 상대운동",45,98,p);
